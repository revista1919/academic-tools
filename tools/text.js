console.log("tools/text.js cargado correctamente (CodeMirror 5 con Quill WYSIWYG simplificado)");
...
\\usepackage{subcaption}`,
  ...
  const preambleForm = document.getElementById('preamble-form');
  const bibForm = document.getElementById('bib-form');
  const latexPreview = document.getElementById('latex-preview');
  const copyLatex = document.getElementById('copy-latex');
  const exportBib = document.getElementById('export-bib');
  const copyPreamble = document.getElementById('copy-preamble');
  ...
  class AbstractBlot extends BlockEmbed {
    ...
    static create(value) {
      let node = super.create();
      node.innerHTML = '<strong>Abstract</strong> ';
  ...
  class TheoremBlot extends BlockEmbed {
    ...
    static create(value) {
      let node = super.create();
      node.innerHTML = '<strong>Theorem</strong> ';
      const content = document.createElement('div');
      content.innerHTML = value || '';
      content.contentEditable = true;
      node.appendChild(content);
  ...
  class LemmaBlot extends BlockEmbed {
    ...
    static create(value) {
      let node = super.create();
      node.innerHTML = '<strong>Lemma</strong> ';
      const content = document.createElement('div');
      content.innerHTML = value || '';
      content.contentEditable = true;
      node.appendChild(content);
  ...
  class ProofBlot extends BlockEmbed {
    ...
    static create(value) {
      let node = super.create();
      node.innerHTML = '<strong>Proof</strong> ';
      const content = document.createElement('div');
      content.innerHTML = value || '';
      content.contentEditable = true;
      node.appendChild(content);
  ...
  if (mainEditorElem) {
    ...
    mainEditor.on("change", () => {
      if (!isVisualMode) {
        updateSidebar();
        updateWordCount();
        if (latexPreview) latexPreview.innerText = mainEditor.getValue();
      }
    });
  ...
  if (visualEditorElem) {
    ...
    quill.on('text-change', () => {
      if (isVisualMode) {
        updateSidebar();
        updateWordCount();
        if (latexPreview) latexPreview.innerText = deltaToLatex(quill.getContents());
      }
      autoSave();
    });
    console.log("Visual editor (Quill) creado con características simplificadas");
  }
  function autoSave() {
    const project = {
  ...
  if (savedProject) {
    ...
    updateSidebar();
    updateWordCount();
    if (latexPreview) latexPreview.innerText = isVisualMode ? deltaToLatex(quill.getContents()) : mainEditor.getValue();
  }
  ...
  function updateWordCount() {
    ...
    if (isVisualMode) {
      ...
      citations = quill.root.querySelectorAll('.citation').length;
    } else {
      text = mainEditor.getValue().replace(/\\[^{}]+/g, '').replace(/\s+/g, ' ').trim();
      citations = (mainEditor.getValue().match(/\\cite/g) || []).length;
    }
    const wordCount = text.split(' ').filter(w => w).length;
    wordCountElem.textContent = `Palabras: ${wordCount} | Citas: ${citations}`;
  }
  if (darkModeToggle) {
  ...
  if (generateStructureButton && templateSelect) {
    generateStructureButton.addEventListener('click', () => {
      ...
      if (isVisualMode) {
        quill.setContents(new Delta().insert('\nTítulo\n', {header: 1}).insert('Escribe un párrafo aquí...\n\n'));
      } else {
        mainEditor.setValue('\\section{Título}\nEscribe un párrafo aquí...');
      }
      bibEditor.setValue('');
      updateSidebar();
      updateWordCount();
      if (latexPreview) latexPreview.innerText = isVisualMode ? deltaToLatex(quill.getContents()) : mainEditor.getValue();
      console.log("Estructura simplificada generada con template:", template);
    });
  }
  ...
  function exportToOnline(serviceUrl) {
    try {
      ...
      navigator.clipboard.writeText(fullTex).then(() => {
        alert('Código LaTeX copiado al portapapeles. Pégalo en el editor de ' + serviceUrl + ' (e.g., Overleaf).');
  ...
  if (preambleForm) {
    preambleForm.addEventListener('submit', (e) => {
      e.preventDefault();
      ...
      const extra = document.getElementById('extra-options').value;
      const customPkgs = document.getElementById('custom-packages').value.split(',').map(p => p.trim()).filter(p => p);
      let opts = [];
      if (paper) opts.push(paper);
      if (fsize) opts.push(fsize);
      let pre = `\\documentclass[${opts.join(',')}]{${cls}}\n\\usepackage[utf8]{inputenc}\n`;
      if (margins) pre += `\\usepackage{geometry}\n\\geometry{${margins}}\n`;
      if (font !== 'default') pre += `\\usepackage{${font}}\n`;
      if (document.getElementById('pkg-amsmath').checked) pre += '\\usepackage{amsmath}\n';
      if (document.getElementById('pkg-natbib').checked) pre += '\\usepackage{natbib}\n';
      if (document.getElementById('pkg-graphicx').checked) pre += '\\usepackage{graphicx}\n';
      if (document.getElementById('pkg-hyperref').checked) pre += '\\usepackage{hyperref}\n';
      if (document.getElementById('pkg-booktabs').checked) pre += '\\usepackage{booktabs}\n';
      if (document.getElementById('pkg-caption').checked) pre += '\\usepackage{caption}\n';
      if (document.getElementById('pkg-subcaption').checked) pre += '\\usepackage{subcaption}\n';
      if (document.getElementById('pkg-setspace').checked) pre += '\\usepackage{setspace}\n\\doublespacing\n';
      customPkgs.forEach(pkg => pre += `\\usepackage{${pkg}}\n`);
      pre += extra;
  ...
  if (bibForm) {
    bibForm.addEventListener('submit', (e) => {
      e.preventDefault();
      const type = document.getElementById('bib-type').value;
      const key = document.getElementById('bib-key').value;
      const author = document.getElementById('bib-author').value;
      const title = document.getElementById('bib-title').value;
      const year = document.getElementById('bib-year').value;
      const extra = document.getElementById('bib-extra').value;
      let bib = `@${type}{${key},\n`;
  ...
  if (copyLatex) {
    copyLatex.addEventListener('click', () => {
      navigator.clipboard.writeText(latexPreview.innerText);
      alert('Código LaTeX copiado!');
    });
  }
  if (exportBib) {
    exportBib.addEventListener('click', () => {
      const blob = new Blob([bibEditor.getValue()], { type: 'text/plain' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'refs.bib';
  ...
  if (copyPreamble) {
    copyPreamble.addEventListener('click', () => {
      navigator.clipboard.writeText(preambleEditor.getValue());
      alert('Configuración del documento (Preamble) copiada!');
    });
  }
});